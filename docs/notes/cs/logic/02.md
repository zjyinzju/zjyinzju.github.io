...
!!! warning "注意"
    ...
    反函数是指将原函数中的 AND 和 OR 互换，同时将所有的常数和变量取反。

    !!! eg "example"
        - 原函数：$F=\overline{A}B+C\overline{B}$；
        - 其对偶函数：$F=(\overline{A}+B)(C+\overline{B})$；
        - 其反函数：$\overline{F}=(A+\overline{B})(\overline{C}+D)$；
...
!!! Example "index" 
    ...
    如果有 n 个变量，则index 从 0 到 $2^n-1$
    
    将index写为二进制数，对应位置的 0 表示取反，1 表示不取反。将**按照字母顺序排列**的变量按照index的二进制数对应位取反或不取反，就可以得到 minterm 和 maxterm 的表达式。
      

对偶关系： $m_i = \overline{M_i}$.   
